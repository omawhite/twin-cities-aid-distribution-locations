...
const getStatus = (id) =>
  _.find(statusOptions, (s) => s.id === id.toLowerCase());

// create an item for the side pane using a location
const createListItem = (location, status, lng, lat) => {
  const urgentNeed = location.urgentNeed
    ? `<p class="urgentNeed p location-list--important">Urgent Need: ${location.urgentNeed}</p>`
    : "";
  const seekingMoney = location.seekingMoney
    ? `<span class="seekingMoney location-list--badge">Needs Money Donations</span>`
    : "";
  const seekingVolunteers = location.seekingVolunteers
    ? `<span class="seekingVolunteers location-list--badge">Needs Volunteer Support</span>`
    : "";
...
// const dataPromise = fetch(process.env.DATA_URL);
...
const onMapLoad = async () => {
  ...
  locations = _.chain(data.feed.entry)
    .filter(
      (item) =>
        item.gsx$nameoforganization.$t != "" &&
        item.gsx$longitude.$t != "" &&
        item.gsx$latitude.$t != ""
    ) // only items with names and lon,lat
    ...
    .map((item) => {
      try {
        ...
        const seekingMoney =
          moneySearchStr.includes("money") ||
          moneySearchStr.includes("cash") ||
          moneySearchStr.includes("venmo") ||
          moneySearchStr.includes("monetary");
        // the location schema
        const rawLocation = {
          name: item.gsx$nameoforganization.$t,
          neighborhood: item.gsx$neighborhood.$t,
          address: item.gsx$addresswithlink.$t,
          currentlyOpenForDistributing:
            item.gsx$currentlyopenfordistributing.$t,
          openingForDistributingDontations:
            item.gsx$openingfordistributingdonations.$t,
          closingForDistributingDonations:
            item.gsx$closingfordistributingdonations.$t,
          ...
          openingForReceivingDontations:
            item.gsx$openingforreceivingdonations.$t,
          closingForReceivingDonations:
            item.gsx$closingforreceivingdonations.$t,
        ...
        const propertyTransforms = {
          ...
          address: (address) =>
            `<address class='p'><a href="https://maps.google.com?saddr=Current+Location&daddr=${encodeURI(
              address
            )}" target="_blank">${address}</a></address>`, // driving directions in google, consider doing inside mapbox
        };
        ...
        const markerHtml = _.map(location, (value, key) => {
          if (propertyTransforms[key]) return propertyTransforms[key](value);
            ...
            return `<p class='p'><span class='txt-deemphasize'>${camelToTitle(
              key
            )}: </span>${value}</p>`;
        }).join("");
        ...
        location.marker = new mapboxgl.Marker({ color: status.accessibleColor })
          ...
          .setPopup(
            new mapboxgl.Popup()
              .setMaxWidth("250px")
              .setHTML(`<div class='popup-content'>${markerHtml}</div>`)
          )
          .addTo(map);
